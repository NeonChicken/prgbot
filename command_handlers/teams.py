...
async def run(client, message):
    ...
    def help_msg():
        ...
        response = "{}".format(message.author.mention) + "" \
                   "\n\n**" + prefix +  "{}** ***(amount of teams)*** *player1 player2 player3 etc.*" \
                                        "\nMakes teams based on your input.".format(commandname) + "" \
                                        "\n\nIf you want a *seperate amount of players* in each team;\nAfter (amount of teams) you can type **true**. Example:" \
                                        "\n" + prefix +  "{} 2 true a b c d e".format(commandname)
    ...
    if len(message.content.split()) >= 2:
            ...
            if message.content.split()[2].lower() == 'true':
                if int(message.content.split()[1]):
                    ...
                    if playerTotal > user_amt:
                        await message.channel.send(
                            "{}, your input exceeds the players you presented!\n".format(message.author.mention))
                        return
        ...
        try:
            ...
            await message.channel.send("{}\n*".format(message.author.mention) + prefix + "{}* ***(amount of teams)*** should be an number divisible by the amount of players you input!".format(commandname))
            return